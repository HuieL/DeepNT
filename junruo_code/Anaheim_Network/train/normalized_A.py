...
df = pd.read_csv('shuffled_data/list_Anaheim_Network.csv')
...
edge_index = np.load('shuffled_data/pair.npy')
...
class GCN(nn.Module):
    ...
    def forward(self,X):
        # print(self.weighted_A.size())
        # print(X.size())
        # print(self.weighted_A.mm(X).size() )
        # print('predict_adj')
        # print(self.weighted_A.weight)
        # self.weighted_A.weight = self.normalize(self.weighted_A.weight)
        X=self.weighted_A(X.t())
        # print('AX1')
        # print(X)
        ...
        # X = F.relu(self.fc1(self.weighted_A.mm(X)))
     
        # X = F.relu(self.fc3(self.weighted_A(X)))
        X = self.weighted_A(X.t())
        # print('AX2')
        # print(X)


        X = F.relu(self.fc3(X.t()))
        # print('WAX2')
        # print(X)
...
f = np.load('shuffled_data/features.npy')
node_features = torch.as_tensor(torch.from_numpy(f), dtype=torch.float32)
features = node_features
X_dim = 9
features = features.cuda()
label_list = np.load('shuffled_data/label_list.npy')
...
path_index = np.load('shuffled_data/pair_aug.npy')
