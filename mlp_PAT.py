...
def load_all(train= True):
    ...
    # ini_term_pair = torch.FloatTensor(ini_term_pair)
    ini_term_pair = ini_term_pair.to(torch.float)
    # print(len(ini_term_pair))

    label_list = np.load('label_list.npy')
    label_list = torch.from_numpy(label_list)
    label_list = label_list.to(torch.float)
    
    # print(len(label_list))

    test_ini_term_pair = np.load('test_ini_term_pair.npy')
    test_ini_term_pair = torch.from_numpy(test_ini_term_pair)
    # test_ini_term_pair = torch.FloatTensor(test_ini_term_pair)
    test_ini_term_pair = test_ini_term_pair.to(torch.float)
...
for ep in range(epochs):
    ...
    for  ini_term_node_test, label_test in test_loader:
        ...
        # print('predict')
        # print(output)
        MAPE = metrics.mean_absolute_percentage_error(label_test.cpu().detach().numpy(), output.cpu().detach().numpy())
        mape.append(MAPE)
    print("epoch {:03d}: mape = {:.3f}".format(ep, np.mean(mape)))
